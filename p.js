...
async function fetchDataAndPopulateGallery(apiUrl) {
  try {
    ...
    const galleryDefaultContainer = document.querySelector('.row-default');
...
function createUpdateButton(authorId) {
  let updateButton = document.createElement('button');
  updateButton.innerHTML = 'Update';
  updateButton.id = `updateButton_${authorId}`;
  updateButton.addEventListener('click', function() {
      updateItem(authorId);
  });
  return updateButton;
...
async function updateItem(authorId) {
  ...
  try {
      ...
      if (!response.ok) {
          console.error('Error fetching item data for update:', response.status, response.statusText);
          return;
      }
